...
DOMAINS = ["uber", "movie", "restaurant", "coffee", "pizza", "auto", "sport",
           "flight", "food-ordering", "hotel", "music"]
...
def prepocess_API(frame):
    if (frame == ""):
        ...
        services = set()
        for s in frame:
            parsed = remove_numbers_from_string(
                s["annotations"][-1]["name"]).split(".")
...
def fix_turn(turns):
    ...
    for i_t, t in enumerate(turns):
        if (t['spk'] == "API-OUT" and t['utt'] != ""):
            if turns[i_t - 1]['utt'] == "":
                new_turns[-1]["utt"] = list(turns[i_t]['n_struct'].keys())[
                                           0] + "()"
                new_turns[-1]["service"] = [
                    list(turns[i_t]['n_struct'].keys())[0]]
...
def get_data(dialogue, year, develop=False):
    ...
    for i_d, d in tqdm(enumerate(dialogue), total=len(dialogue)):
        ...
        dial = {"id": d["conversation_id"].strip(),
                "services": [f"TM{year}_" + serv], "dataset": f"TM{year}"}
        turns = []
        for t_idx, t in enumerate(d["utterances"]):
            if (t["speaker"] == "USER"):
                if (len(turns) != 0 and turns[-1]['spk'] == "API"):
                    ...
                    turns.append({"dataset": f"TM{year}",
                                  "id": d["conversation_id"].strip(),
                                  "turn_id": t_idx, "spk": t["speaker"],
                                  "utt": t["text"]})
                    turns.append({"dataset": f"TM{year}",
                                  "id": d["conversation_id"].strip(),
                                  "turn_id": t_idx, "spk": "API", "utt": "",
                                  "struct": t[
                                      "segments"] if "segments" in t else "",
                                  "service": []})
            else:
                if (len(turns) != 0 and turns[-1]['spk'] == "SYSTEM"):
                    ...
                    turns.append({"dataset": f"TM{year}",
                                  "id": d["conversation_id"].strip(),
                                  "turn_id": t_idx, "spk": "API-OUT", "utt": "",
                                  "struct": t[
                                      "segments"] if "segments" in t else "",
                                  "service": []})
                    turns.append({"dataset": f"TM{year}",
                                  "id": d["conversation_id"].strip(),
                                  "turn_id": t_idx, "spk": "SYSTEM",
                                  "utt": t["text"]})
...
def preprocessTM2019(develop=False):
    ...
    for dom, data in data_by_domain.items():
        train_data, dev_data, test_data = np.split(data, [int(len(data) * 0.8),
                                                          int(len(data) * 0.9)])
        data_by_domain_new[str(sorted(
            [remove_numbers_from_string(s) for s in eval(dom)]))].append(
    ...
    for dom, list_of_split in data_by_domain_new.items():
        ...
        table.append({"dom": dom, "train": len(train), "valid": len(valid),
                      "test": len(test)})
        train_data += train
        valid_data += valid
        test_data += test
