...
allowed_ACT_list = ["INFORM", "CONFIRM", "OFFER", "NOTIFY_SUCCESS",
                    "NOTIFY_FAILURE", "INFORM_COUNT"]
    ...
    for i_f, f in tqdm(enumerate(files), total=len(files)):
        if "schema.json" not in f:
            ...
            for d in dialogue:
                ...
                dial = {"id": d["dialogue_id"],
                        "services": ["SGD_" + s for s in d["services"]],
                        "dataset": "SGD"}
                turns = []
                dst_prev = []
                for t_idx, t in enumerate(d["turns"]):
                    if (t["speaker"] == "USER"):
                        turns.append({"dataset": "SGD", "id": d["dialogue_id"],
                        ...
                        for intent, slots in dst_api.items():
                            ...
                            for s, v in slots.items():
                                if (s != 'none'):
                                    ...
                                    intent_list.add(
                                        remove_numbers_from_string(intent))
                            if (str_API[-1] == ","):
                                str_API = str_API[:-1]
                            str_API += ") "
                        turns.append({"dataset": "SGD", "id": d["dialogue_id"],
                                      "turn_id": t_idx, "spk": "API",
                                      "utt": str_API,
                                      "service": list(intent_list),
                                      "service_dom": serv})
                    else:
                        ...
                        turns.append({"dataset": "SGD", "id": d["dialogue_id"],
                                      "turn_id": t_idx, "spk": "API-OUT",
                                      "utt": str_ACT, "service": serv})
                        turns.append({"dataset": "SGD", "id": d["dialogue_id"],
                                      "turn_id": t_idx, "spk": t["speaker"],
                                      "utt": t["utterance"]})
                dial["dialogue"] = turns
...
def preprocessSGD(develop=False):
    ...
    for dom, data in data_by_domain.items():
        train_data, dev_data, test_data = np.split(data, [int(len(data) * 0.7),
                                                          int(len(data) * 0.8)])
