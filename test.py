...
def get_example_inputs(model, tokenizer, prompt_text, device):
    ...
    return input_ids.to(device), attention_mask.to(device), position_ids.to(
        device), empty_past


def test_generation_GPT2BATCH(model, tokenizer, input_text, device,
                              do_sample=False, temperature=1.0, top_k=0,
                              top_p=0, max_length=30, task_id=-1):
    eos_token_id = tokenizer.eos_token_id

    input_ids, attention_mask, position_ids, past = get_example_inputs(model,
                                                                       tokenizer,
                                                                       input_text,
                                                                       device)
    batch_size = input_ids.size(0)
    ...
    for step in range(max_length):
        ...
        if do_sample:
            ...
            next_token_logscores = top_k_top_p_filtering(next_token_logits,
                                                         top_k=top_k,
                                                         top_p=top_p)
            # Sample
...
def generate_sample_prev_task(args, model, tokenizer, dataset_dic,
                              task_id_so_far, number_of_sample, time,
                              task_id_adpt=-1):
    ...
    temp_aug_mem = random.sample(dataset_dic["['sgd_restaurants']"],
                                 min(len(dataset_dic["['sgd_restaurants']"]),
                                     number_of_sample))
    ...
    with torch.no_grad():
        if "gpt2" in args.model_checkpoint:  ## this works only with GPT2
            ...
            for i in range(int(number_of_sample / (args.valid_batch_size)) + 1):
                if (
                        i % 2 == 0 or args.task_type != "E2E"):  # sample on batch with and one without API call
                    input_batch = [f"[{str(eval(task_id_so_far)[0])}]" for _ in
                                   range(args.valid_batch_size)]
                else:
                    input_batch = [f"[{str(eval(task_id_so_far)[0])}-API]" for _
                                   in range(args.valid_batch_size)]
                ...
                sample_list += samples
    sample_list = random.sample(sample_list,
                                min(len(sample_list), number_of_sample))
    # this sample is to train the previous task generator
    for i in range(len(temp_aug_mem)):
        temp_aug_mem[i][
            "history_reply"] = f"{sample_list[i].strip()} {tokenizer.eos_token}"
    ...
    with open(f'{args.saving_dir}/{time}' + f'/{task_id_so_far}_generated.json',
              'w') as fp:
...
def test_model_seq2seq(args, model, tokenizer, test_loader, time="0_['']"):
        ...
        with torch.no_grad():
            if "gpt2" in args.model_checkpoint:
                ...
                responses = model.generate(
                    ...
                    eos_token_id=tokenizer.eos_token_id,
                    max_length=100)
                value_batch = tokenizer.batch_decode(responses,
                                                     skip_special_tokens=True)
        for idx, resp in enumerate(value_batch):
            results.append(
                {"id": batch["dial_id"][idx], "turn_id": batch["turn_id"][idx],
    ...
    if not os.path.exists(f'{args.saving_dir}/{time}'):
        os.makedirs(f'{args.saving_dir}/{time}')
    with open(f'{args.saving_dir}/{time}' + '/generated_responses.json',
              'w') as fp:
...
def test_model_seq2seq_ADAPTER(args, model, tokenizer, test_loader,
                               test_dataset, time="0_['']", max_seen_task=0):
    # device = torch.device(f"cuda:{args.GPU[0]}")
    device = torch.device(f"cuda:0")
    ...
    perplexity_dict = {
        f'{sample["dial_id"]}_{sample["turn_id"]}_{sample["task_id"]}': [] for
        sample in test_dataset}
    for t in range(range_adpt):
        print(f"Task {t}")
        for idx_b, batch in tqdm(enumerate(test_loader),
                                 total=len(test_loader)):
            ppl_batch = model.compute_PPL(batch, task_id=t,
                                          device=device)  ## one value per batch
            for (d_id, t_id, ta_id, ppl) in zip(batch["dial_id"],
                                                batch["turn_id"],
                                                batch["task_id"], ppl_batch):
    ...
    for sample in test_dataset:
        if (
                f'{sample["dial_id"]}_{sample["turn_id"]}_{sample["task_id"]}' in perplexity_dict):
            test_dataset_by_predicted_id[perplexity_dict[
                f'{sample["dial_id"]}_{sample["turn_id"]}_{sample["task_id"]}']].append(
                sample)
    ...
    test_dataset_by_predicted_id = {k: make_loader(args, v, model.tokenizer) for
                                    k, v in
                                    test_dataset_by_predicted_id.items()}

    for pred_task_id, task_loader in tqdm(test_dataset_by_predicted_id.items(),
                                          total=len(
                                                  test_dataset_by_predicted_id)):
        # print(f"Task Id: {task_id}")
        for idx_b, batch in tqdm(enumerate(task_loader),
                                 total=len(task_loader)):
            with torch.no_grad():
            ...
            for idx, resp in enumerate(value_batch):
                results.append({"id": batch["dial_id"][idx],
