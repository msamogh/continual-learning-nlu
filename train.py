...
from test import test_model_seq2seq, generate_sample_prev_task, \
    test_model_seq2seq_ADAPTER
from utils.dataloader import get_data_loaders, get_current_task_data, \
    make_loader
...
def get_checkpoint(log_dir, index_to_load):
    ...
    for f in file:
        ...
        if index_to_load in num:
            ...
            check_name = \
            os.listdir(f + "/lightning_logs/" + version + "/checkpoints/")[0]
...
def train(hparams, *args):
    ...
    train_loader, val_loader, dev_val_loader, (
    train_datasets, val_datasets, test_datasets) = get_data_loaders(hparams,
                                                                    model.tokenizer)
    ...
    task_seen_so_far = []
    if (hparams.CL != "MULTI"): model.set_number_of_tasks(
        len(list(train_loader.keys())))
    ...
    if hparams.multi:
        ...
        trainer = Trainer(
            ...
            callbacks=[
                pl.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.00,
        ...
        for task_num, (task_id, task_loader) in enumerate(train_loader.items()):
            ...
            if (hparams.CL == "REPLAY"):
                ...
                task_loader = make_loader(hparams, train_datasets[
                    task_id] + model.reply_memory, model.tokenizer)

            if (hparams.CL == "LAMOL"):
                if (
                        current_task_to_load == None or task_num >= current_task_to_load):
                    number_of_sample = hparams.percentage_LAM0L
                    aug_current_task = get_current_task_data(hparams,
                                                             train_datasets[
                                                                 task_id],
                                                             task_id,
                                                             number_of_sample)
                    ...
                    for task_id_so_far in task_seen_so_far:
                        ...
                        temp = generate_sample_prev_task(hparams, model.model,
                                                         model.tokenizer,
                                                         train_datasets,
                                                         task_id_so_far,
                                                         number_of_sample,
                    ...
                    task_loader = make_loader(hparams, train_datasets[
                        task_id] + aug_current_task + aug_data_prev_task,
                                              model.tokenizer)
            ...
            if (hparams.CL != "LAMOL" and hparams.CL != "EWC"):
                ...
                checkpoint['state_dict'] = {k.replace('model.', ''): v for k, v
            ...
            if (hparams.test_every_step):
                if (hparams.CL == "ADAPTER"):
                    test_model_seq2seq_ADAPTER(hparams, model, model.tokenizer,
                                               dev_val_loader, test_datasets,
                                               time=f"{task_num}_{task_id}")
                else:
                    test_model_seq2seq(hparams, model.model, model.tokenizer,
                                       dev_val_loader,
            ...
            if hparams.CL == "AGEM":
                ...
                # in percentage
                random.seed(52)
                model.reply_memory += random.sample(train_datasets[task_id],
                                                    min(len(train_datasets[
                                                                task_id]),
            ...
            if hparams.CL == "EWC" or hparams.CL == "L2":
                ...
                for n, p in model.model.named_parameters():
                    ...
                    model.fisher[n] = torch.zeros(
                        p.size())  # torch.Tensor(p.cpu().data).zero_()
                ...
                if hparams.CL == "EWC":
                    ...
                    for name_f, _ in model.fisher.items():
                        model.fisher[name_f] /= len(model.episodic_mem[
                                                        task_id])  # *hparams.train_batch_size
                    model.model.zero_grad()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_list", type=str,
                        default="SGD,TM19,TM20,MWOZ", help="Path for saving")
    ...
    parser.add_argument("--reg", type=float, default=0.01,
                        help="CL regularization term")
    parser.add_argument("--episodic_mem_size", type=int, default=100,
