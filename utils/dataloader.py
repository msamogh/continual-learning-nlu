                ...
                dialogues.append(
                    {"history": " ".join(plain_history[-args.max_history:]),
                     ...
                     "history_reply": " ".join(plain_history[
                                               -args.max_history:]) + f'[SOS]{intent} {tokenizer.eos_token}',
                     "spk": t["spk"],
                     "dataset": t["dataset"],
                     "dial_id": t["id"],
                     "turn_id": t["turn_id"],
            ...
            if (t['spk'] == "USER"):
                ...
                dialogues.append(
                    {"history": " ".join(plain_history[-args.max_history:]),
                     ...
                     "history_reply": " ".join(plain_history[
                                               -args.max_history:]) + f'[SOS]{slots} {tokenizer.eos_token}',
                     "spk": t["spk"],
                     "dataset": t["dataset"],
                     "dial_id": t["id"],
                     "turn_id": t["turn_id"],
    ...
    for dial in data:
        ...
        for idx_t, t in enumerate(dial['dialogue']):
            ...
            if (t['spk'] == "USER"):
                ...
                dialogues.append({"history": " ".join(
                    plain_history[-args.max_history:] + [latest_API_OUT]),
                                  "reply": f'{t["utt"].strip()} {tokenizer.eos_token}',
                                  "history_reply": " ".join(
                                      plain_history[-args.max_history:] + [
...
def get_current_task_data(args, dataset_dic, task_id, number_of_sample):
    temp_aug = random.sample(dataset_dic,
                             min(number_of_sample, len(dataset_dic)))
    aug_data = []
    cnt_API = 0
    for d in temp_aug:
        ...
        if (args.task_type == "E2E"):
            if (d["spk"] == "API"):
                cnt_API += 1
                d["history_reply"] = f"[{str(eval(task_id)[0])}-API]" + d[
                    "history_reply"]
            else:
                d["history_reply"] = f"[{str(eval(task_id)[0])}]" + d[
                    "history_reply"]
        else:
            d["history_reply"] = f"[{str(eval(task_id)[0])}]" + d[
                "history_reply"]
...
def collate_fn(data, tokenizer):
    ...
    input_batch = tokenizer(batch_data["history"], padding=True,
                            return_tensors="pt", truncation=False,
                            add_special_tokens=False)
    batch_data["encoder_input"] = input_batch["input_ids"]
    batch_data["attention_mask"] = input_batch["attention_mask"]
    output_batch = tokenizer(batch_data["reply"], padding=True,
                             return_tensors="pt", truncation=False,
                             add_special_tokens=False,
...
def collate_fn_GPT2(data, tokenizer):
    ...
    input_batch = tokenizer(batch_data["history_reply"], padding=True,
                            return_tensors="pt", truncation=False,
                            add_special_tokens=False,
                            return_attention_mask=False)
    batch_data["encoder_input"] = input_batch["input_ids"]
    batch_data["attention_mask"] = None
    output_batch = tokenizer(batch_data["history_reply"], padding=True,
                             return_tensors="pt", truncation=False,
                             add_special_tokens=False,
                             return_attention_mask=False)
    # replace the padding id to -100 for cross-entropy
    ...
    batched_history = tokenizer(batch_data["history"], padding=True,
                                return_tensors="pt", truncation=False,
                                add_special_tokens=False,
                                return_attention_mask=False)
    batch_data["input_id_PPL"] = batched_history['input_ids']
    batched_history_out = tokenizer(batch_data["history"], padding=True,
                                    return_tensors="pt", truncation=False,
                                    add_special_tokens=False,
                                    return_attention_mask=False)
    ...
    batch_data["output_id_PPL"] = batched_history_out[
        'input_ids']  ### basically just remove pad from ppl calculation
    return batch_data


def make_loader(args, list_sample, tokenizer):
    collate_fn_ = collate_fn_GPT2 if (
                "gpt2" in args.model_checkpoint) else collate_fn
    return DataLoader(DatasetTrain(list_sample),
...
def get_data_loaders(args, tokenizer, test=False):
    ...
    aggregate = get_datasets(dataset_list=args.dataset_list.split(','),
                             setting=args.setting, verbose=args.verbose,
                             develop=args.debug)
    ...
    for split in datasets.keys():
        for tasks_id, task in aggregate["BYDOMAIN"][split].items():
            if (args.task_type == "E2E"):
                datasets[split][tasks_id] = get_e2e_from_dial(args, task,
                                                              tasks_id,
                                                              tokenizer)
            elif (args.task_type == "INTENT"):
                datasets[split][tasks_id] = get_intent_from_dial(args, task,
                                                                 tasks_id,
                                                                 tokenizer)
            elif (args.task_type == "DST"):
                datasets[split][tasks_id] = get_DST_from_dial(args, task,
                                                              tasks_id,
                                                              tokenizer)
            elif (args.task_type == "NLG"):
                datasets[split][tasks_id] = get_NLG_from_dial(args, task,
                                                              tasks_id,
                                                              tokenizer)
    ...
    task_id_dev = set(
        task_id for task_id, dataset_task in datasets["dev"].items())
    task_id_test = set(
        task_id for task_id, dataset_task in datasets["test"].items())
    ...
    for dom_name, split_len in task.items():
        table.append({"dom": dom_name, "train": split_len["train"],
                      "dev": split_len["dev"], "test": split_len["test"]})
    ...
    if (args.continual):
        ...
        if (not test):
            ...
            train_loaders = DataLoader(DatasetTrain(dataset_train),
            ...
            valid_loaders = DataLoader(DatasetTrain(dataset_dev),
    ...
    test_loaders = DataLoader(DatasetTrain(sum(temp_list, [])),
                              batch_size=args.valid_batch_size, shuffle=False,
                              collate_fn=partial(collate_fn_,
                                                 tokenizer=tokenizer))
    ...
    for idx_b, batch in tqdm(enumerate(test_loaders), total=len(test_loaders)):
        for (d_id, t_id, ta_id) in zip(batch["dial_id"], batch["turn_id"],
                                       batch["task_id"]):
