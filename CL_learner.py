...
from transformers import (AdamW, GPT2Tokenizer, GPT2LMHeadModel, T5Tokenizer,
...
class Seq2SeqToD(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        if "t5" in args.model_checkpoint:
            ...
            if (args.CL == "ADAPTER"):
                ...
                model.add_adapters(bottleneck_size=args.bottleneck_size,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.CL == "GEM" and not self.first_task:
            dev = next(self.model.parameters()).device
            for id_task, (_, task_memory) in enumerate(
                    self.episodic_mem.items()):
                batch_mem = sample(task_memory, 1)[
                    0]  # ==> we sample one batch from episodic memory
        ...
        if (self.CL == "AGEM" and not self.first_task):
            ...
            if angle < 0:
                ...
                for p in self.model.parameters():
                    if p.requires_grad:
                        ...
                        p.grad.copy_(
                            grad_proj[index:index + n_param].view_as(p))
                        index += n_param
            ...
            if (dotp < 0).sum() != 0:
                ...
                overwrite_grad(self.model.parameters,
                               self.grads.to(dev)[:, id_task], self.grad_dims)
            ...
            for n, p in self.model.named_parameters():
                ...
                l = self.reg * self.fisher[n].to(dev) * (
    ...
    def validation_step(self, batch, batch_idx):
        if (self.CL == "ADAPTER"):
            (loss), *_ = self.model(input_ids=batch["encoder_input"],
                                    attention_mask=batch["attention_mask"],
                                    labels=batch["decoder_output"],
                                    task_id=self.task_list_seen.index(
                                        batch["task_id"][0])
                                    )
        else:
            print(batch["encoder_input"].size())
    ...
    def configure_optimizers(self):
        if (self.CL == "ADAPTER"):
            parameters_to_update = [p for n, p in self.named_parameters() if
                                    "adapter" in str(n)]
