...
def evaluate_API(pred, gold):
    ...
    for p, g in zip(pred, gold):
        ...
        if (g != ""):
            ...
            # print(API_G)
            if (
                    p != "" and "(" in p and ")"):  ## means the predicted text is an API
                API_P = parse_API(p)
                if len(API_G.keys()) != 1:
                    continue
                if len(API_P.keys()) != 1:
                ...
                state_G = {s: v for s, v in API_G[intent_G].items() if
                           s != "none"}
                state_P = {s: v for s, v in API_P[intent_P].items() if
                           s != "none"}
...
def evaluate_EER(args, results_dict, entities_json, path, names):
    ...
    for d in results_dict:
        if (d["spk"] == "SYSTEM"):
            ...
            for speech_act, slot_value_dict in parse_API(
                    d['hist'] + " ").items():
...
def evaluate(args, path, names, ent={}):
    ...
    for r in results_json:
        if (r['spk'] == 'SYSTEM'):
            ...
            domain_BLEU[r['task_id']]["gold"].append(
                r['gold'].replace("[eos]", "").strip())
    ...
    if args.task_type == "NLG" or args.task_type == "E2E":
        for k, sample_NLG in domain_NLG.items():
            T_NLG[k] = evaluate_EER(args, sample_NLG, entities_json, path,
                                    names)
...
perm1 = {0: "['sgd_travel']", 1: "['sgd_payment']", 2: "['TMA_restaurant']",
         3: "['TMB_music']", 4: "['sgd_ridesharing']", 5: "['TMA_auto']",
         6: "['sgd_music']", 7: "['sgd_buses']", 8: "['TMB_restaurant']",
         9: "['MWOZ_attraction']", 10: "['TMB_sport']", 11: "['sgd_movies']",
         12: "['sgd_homes']", 13: "['TMA_coffee']", 14: "['sgd_restaurants']",
         15: "['sgd_hotels']", 16: "['sgd_weather']", 17: "['sgd_trains']",
         18: "['MWOZ_train']", 19: "['sgd_flights']", 20: "['sgd_media']",
         21: "['MWOZ_taxi']", 22: "['sgd_alarm']", 23: "['TMA_movie']",
         24: "['sgd_banks']", 25: "['TMA_pizza']", 26: "['TMB_flight']",
         27: "['sgd_rentalcars']", 28: "['TMB_movie']", 29: "['sgd_events']",
         30: "['MWOZ_restaurant']", 31: "['sgd_services']",
         32: "['sgd_calendar']", 33: "['TMB_food-ordering']",
         34: "['MWOZ_hotel']", 35: "['TMA_uber']", 36: "['TMB_hotel']"}
...
def score_folder():
    ...
    parser.add_argument("--model_checkpoint", type=str, default="",
                        help="Path to the folder with the results")
    parser.add_argument("--task_type", type=str, default="E2E",
                        help="Path to the folder with the results")
    ...
    for folder in folders:
        if "png" in folder or "TOO_HIGH_LR" in folder or "TEMP" in folder:
            continue
        res = evaluate(args, f'{folder}/FINAL/generated_responses.json',
                       names)  # , ent=entities_json)
        if (args.task_type == "INTENT"):
            INTENT = np.mean(
                [v["intent_accuracy"] for k, v in res["API"].items()])
            RESULT.append(
                {"Name": folder.split("/")[-1].split("_")[0], "INTENT": INTENT})
        elif (args.task_type == "DST"):
            JGA = np.mean(
                [v["turn_level_joint_acc"] for k, v in res["API"].items()])
            RESULT.append(
                {"Name": folder.split("/")[-1].split("_")[0], "JGA": JGA})
        elif (args.task_type == "NLG"):
            BLEU = np.mean([v for k, v in res["BLEU"].items()])
            EER = np.mean([v for k, v in res["EER"].items()])
            RESULT.append(
                {"Name": folder.split("/")[-1].split("_")[0], "BLEU": BLEU,
                 "EER": EER})
